...
{   
    ...
    public Phylum[] habitat;                                            // Array of Habitat to be randomized
    public Phylum[] diet;                                               // Array of Diet to be randomized
    ...
    private int seeMode = 0;                                                // Determines the mode of the see game; 0 phylum, 1 habitat, 2 diet
    ...
    public GameObject SpawnGameAnimals()
    {
        ...
        if (gameType == ScoreManager.MinigameType.SEE && phyla.Length > 1)
        {
            ...
            {
                case 0:
                    correct = SpawnAnimalsSee(phyla);
                    break;
                case 1:
                    correct = SpawnAnimalsSee(habitat);
                    break;
                case 2:
                    correct = SpawnAnimalsSee(diet);
                    break;
                default:
                    correct = null;
                    break;
            ...
            seeMode++;          // Incerement see mode to loop through the cases
            if (seeMode == 3)
            {
                seeMode = 0;    // Reset SeeMode to default case
            }
        ...
        else
        {
            ...
            // Reset the gaze component into true
            foreach (var a in animals)
            {
                a.GetComponent<Gaze>().enabled = true;
            }
    ...
    // Handles Spawning for different Animals in See
    // This takes a array of Phylum to know where the
    // function will get the different phylum it will spawn
    private GameObject SpawnAnimalsSee(Phylum[] animalSet)
    {
        int phylumNum = Random.Range(0, animalSet.Length);                              // Choose a random phylum to instantiate animals from
        Phylum phylum = animalSet[phylumNum];                                               // Assign random to var phylum
        phylum.EnlargeAnimals();
        // Get random phylum to instantiate extra animal
        int extraPhylumNum;                                                             // Get an random int to assign extra phylum from
        do
        {
            extraPhylumNum = Random.Range(0, animalSet.Length);                             // Must not be equal to original phylum
        } while (extraPhylumNum == phylumNum);
        Phylum extraPhylum = animalSet[extraPhylumNum];                                     // Using random int assign the extra phlyum
        extraPhylum.EnlargeAnimals();
    ...
    private GameObject SpawnGameObjectList(GameObject[] list, GameObject correct)
    {
        ...
        for (int i = 0; i < list.Length; i++)
        {
            ...
            // Enlarge this animal if neede
            if (tmp.GetComponent<Animal>().Enlarge)
            {
